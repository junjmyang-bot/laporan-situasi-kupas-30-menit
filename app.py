...
ACTIVITY_GROUP_TEMPLATES = {
    "IS": ["Kupas", "Ceking", "Geprek", "Pritil nata"],
    "Potong": ["PPMH", "PPBAR", "Potong PPMH", "Potong PPBAR"],
    "Jagung": ["Potong jagung"],
...
def validate(payload: dict) -> list[str]:
    ...
    required = [
        ...
        ("checker_kupas", "Petugas cross cek kupas wajib diisi."),
...
def main() -> None:
    ...
    st.title("B-1-2 Laporan Situasi (Kupas)")
    ...
    st.caption("Singkatan: k=Kupas, lk=Kupas laki-laki, c=Cuci, pk=Packing, gd=Gudang, dr=Dry, st=Steam. Contoh input: 3k+2gd")
    if "manual_groups" not in st.session_state:
        st.session_state["manual_groups"] = []
        ...
        with st.expander(f"- {group}", expanded=True):
            ...
            with add_c1:
                new_name = st.text_input(
                    ...
                    placeholder="contoh: Ceking / Kupas 2",
                    label_visibility="collapsed",
                )
            with add_c2:
